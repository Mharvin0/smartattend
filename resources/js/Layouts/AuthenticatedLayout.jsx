	...
	return (
		<div className="flex min-h-screen bg-gray-50">
			...
			<div className={`fixed inset-y-0 z-50 flex flex-col bg-white transition-all duration-300 ${
				...
				<div className="border-t border-gray-200 p-4">
					<div className="flex items-center gap-3">
						<div className="h-10 w-10 flex-shrink-0 overflow-hidden rounded-full bg-brand-secondary">
						...
						{sidebarOpen && (
							<div className="min-w-0 flex-1">
								...
								<p className="truncate text-sm text-gray-500">{roles.join(', ')}</p>
							</div>
						)}
					</div>
				</div>
			...
			<div className={`flex-1 transition-all duration-300 ${sidebarOpen ? 'ml-64' : 'ml-20'}`}>
				...
				<main className="page mx-auto px-4 py-6 sm:px-6 lg:px-8">{children}</main>
			</div>
		</div>
	);
}