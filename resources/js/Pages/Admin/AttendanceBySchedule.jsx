...
export default function AttendanceBySchedule({ schedules, students, attendanceRecords, filters }) {
    ...
    const selectedSchedule = schedules.find(s => s.id == data.schedule_id);
        ...
        <AuthenticatedLayout header={<h2 className="text-2xl font-bold leading-tight text-gray-800">Attendance by Schedule</h2>}>
            ...
            <div className="min-h-screen bg-gradient-to-br from-brand-primary/10 via-emerald-50/80 to-brand-secondary/5 py-8">
                <div className="mx-auto max-w-full space-y-8 px-4 sm:px-6 lg:px-8 xl:px-12">
                    ...
                    <div className="card">
                        <div className="mb-6">
                            <h3 className="text-lg font-medium text-gray-900">Select Schedule and Date</h3>
                            <p className="mt-1 text-sm text-gray-600">Choose the schedule and date for attendance recording.</p>
                        </div>
                        ...
                        <form onSubmit={submit} className="grid grid-cols-1 gap-6 md:grid-cols-3">
                                ...
                                >
                                    ...
                                    {schedules.map((schedule) => (
                                        <option key={schedule.id} value={schedule.id}>
                                            {schedule.subject?.name} - {schedule.section?.name} ({schedule.day} {schedule.time_start}-{schedule.time_end})
                                        </option>
                                    ))}
                                </select>
                                {errors.schedule_id && <p className="mt-1 text-sm text-red-600">{errors.schedule_id}</p>}
                            </div>
                            ...
                            <div className="flex items-end">
                                <button
                                    type="button"
                                    onClick={() => router.get(route('admin.attendance.schedule'), { schedule_id: data.schedule_id, date: data.date })}
                        ...
                        <div className="card">
                            ...
                            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                                <div>
                                    ...
                                    <p className="text-lg font-semibold text-gray-900">{selectedSchedule.subject?.name}</p>
                                    <p className="text-sm text-gray-600">{selectedSchedule.subject?.code}</p>
                                </div>
                                <div>
                                    <p className="text-sm font-medium text-gray-500">Section</p>
                                    <p className="text-lg font-semibold text-gray-900">{selectedSchedule.section?.name}</p>
                                </div>
                                <div>
                                    <p className="text-sm font-medium text-gray-500">Day & Time</p>
                                    <p className="text-lg font-semibold text-gray-900">{selectedSchedule.day}</p>
                                    <p className="text-sm text-gray-600">{selectedSchedule.time_start} - {selectedSchedule.time_end}</p>
                                </div>
                                <div>
                                    <p className="text-sm font-medium text-gray-500">Department</p>
                                    <p className="text-lg font-semibold text-gray-900">{selectedSchedule.department?.name}</p>
                                    <p className="text-sm text-gray-600">{selectedSchedule.program?.name}</p>
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {students.length > 0 && (
                        <div className="card">
                            <div className="mb-6">
                                ...
                                <p className="mt-1 text-sm text-gray-600">Mark attendance for each student in the selected schedule.</p>
                            </div>
                    ...
                    {/* No Students Message */}
                    {data.schedule_id && students.length === 0 && (
                        <div className="card">
                            <div className="text-center py-12">
                                ...
                                <p className="mt-1 text-sm text-gray-500">This schedule doesn't have any students enrolled.</p>
                            </div>
                        </div>
                    )}
                </div>
